...
<br>
	<div class="head">삭제할 리스트의 제목을 클릭해주세요</div>
	<table>
	<tr>
		<th>No.</th>
	...
	<c:if test="${m.updateDate.second ne 0 }">
		...
		<tr>
			<td>${m.marvelId }</td>
			<td><a href="/marvel/deleteUpdate?marvelId=${m.marvelId }"><span>${m.title }</span><br><span>${m.titleEng }</span></a></td>
			<td>${m.releaseDate }</td>
	...
	console.log(spanArr);
	aArr.onclick=function(){
		alert("영화 리스트가 삭제되었습니다.");
	}
</script>	
